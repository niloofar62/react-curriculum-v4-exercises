...
Run `npm run lint` before opening a pull request to ensure no linting errors are introduced.
...
Each week must contain exactly two files:

```
src/exercises/week-[XX]/
├── instructions.md   # Student-facing narrative and task list
└── studentWork.jsx   # Starter file students edit
```
...
- Break tasks into numbered steps under a **Tasks** (or equivalent) section.
...
- Test files in `src/tests/` must cover all required tasks. Write or update tests alongside exercise changes.
...
2. **Create a branch** off `main` following the naming convention above.
